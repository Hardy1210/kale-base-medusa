// Datos de DEMO: categorías, tipos, colecciones, materiales, colores y
// productos del starter (sofás). SOLO PARA LOCAL — nunca en producción.
//
// Requiere haber ejecutado antes seed-config.ts (canal de venta y perfil de
// envío). `corepack yarn seed` lanza los dos en orden.
//
// No es idempotente: crea con handles fijos, así que una segunda ejecución
// falla por duplicado. Para repetirlo, parte de una BD vacía.
//
...
// Debe coincidir con SALES_CHANNEL_NAME de seed-config.ts.
...
export default async function seedDemoData({ container }: ExecArgs) {
  ...
  if (!defaultSalesChannel.length || !shippingProfile) {
    throw new Error(
      'Falta la configuración base de la tienda. Ejecuta primero "corepack yarn seed:config".',
    );
  }
